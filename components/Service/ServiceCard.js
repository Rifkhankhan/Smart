...
const ServiceCard = ({ service }) => {
  ...
  const productImage =
    // service?.images?.length > 0 ? { uri: service.images[0] } : defaultImage;
    ...
    <TouchableOpacity style={styles.card} onPress={detailsHandler}>
      ...
      <View style={styles.details}>
        ...
        <View style={styles.reviewStars}>
          ...
          <Text style={styles.reviewRate}>{service?.ratings}(71)</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  image: {
    height: 170,
    width: "100%",
    resizeMode: "cover", // Adjust the image aspect ratio
  },
  details: {
    flex: 1,
    paddingHorizontal: 8,
