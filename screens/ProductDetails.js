...
const ProductDetails = ({ route, navigation }) => {
	...
	const shop = shops?.filter(shop => shop.shopKey === product.shop)[0]

	const productsInSameStore = products
		?.filter(pro => pro.shop === product?.shop)
	...
	const addToCartHandler = useCallback(async () => {
		...
		const cartExists = carts.some(cart => cart.productKey === data.productKey)
