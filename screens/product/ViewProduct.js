...
import { deleteUser, updateCutomer } from "./../../utils/actions/userActions";
import { useDispatch, useSelector } from "react-redux";
import { deleteProduct, updateProduct } from "../../utils/actions/productActions";
import ProductImageCarousel from "./ProductImageCarousel";
...
const ViewProduct = ({ route, navigation }) => {
  ...
  }, [navigation, product?.name, editHandler]);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.section}>
        ...
        <DetailItem label="Orders" value="5" />
        <DetailItem label="Rank" value={product?.rank} />
        <DetailItem label="Status" value={product?.status ? "Active" : "Blocked"} />
      </View>
