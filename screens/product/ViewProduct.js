...
const ViewProduct = ({ route, navigation }) => {
	...
	const toggleBlock = async () => {
		try {
			...
			await updateProduct(product?.shop, product?.productKey, productData)
	...
	const deleteUserHandler = async () => {
		try {
			const action = await deleteProduct(product?.shop, product.productKey)
			dispatch(action)
			navigation.navigate('ProductListPage')
		} catch (error) {
	...
	return (
		<ScrollView contentContainerStyle={styles.container}>
			...
			<View style={styles.section}>
				...
				{authData.role === 'admin' && (
					<Button
						title={product?.status ? 'Block the user' : 'Unblock the user'}
						color="#333"
						onPress={toggleBlock}
					/>
				)}
