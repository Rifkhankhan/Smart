...
const SearchScreen = ({ navigation }) => {
  ...
  const handleSearch = useDebouncedCallback(async (searchTerm) => {
    if (searchTerm && searchTerm.length > 0) {
      const result = await searchItems(searchTerm, "products");
    ...
    <PageContainer>
      <View style={styles.searchContainer}>
        ...
        <View style={styles.inputContainer}>
          <Ionicons name="search-outline" style={styles.searchIcon} size={18} color="lightgray" />
          <TextInput
            value={searchTerm}
            onChangeText={(text) => {
              setSearchTerm(text);
        ...
        <TouchableOpacity
          onPress={() =>
            searchTerm.length > 0 && searchedProducts.length > 0 && navigation.navigate("SearchedProductsScreen", {
              products: searchedProducts,
              searchTerm,
            })
          }
      ...
      {!noResultsFound && (
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Text style={styles.listItemText}>{renderHighlightedText(item.name, searchTerm)}</Text>
              <Feather name="arrow-up-left" size={15} color="gray" />
            </TouchableHighlight>
          )}
        />
