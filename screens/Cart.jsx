...
const Cart = ({ route, navigation }) => {
  ...
  const selectedItemHandler = useCallback((qty, cart) => {
    setSelectedItemList((prevList) => {
      const itemIndex = prevList.findIndex(
        (product) => product.productKey === cart.productKey
      );
      ...
      if (itemIndex > -1) {
        if (!selectAll) {
          setTotal((prev) => +prev - +totalPrice);
          return prevList.filter((product) => product.productKey !== cart.productKey);
        }
        return prevList;
      } else {
        setTotal((prev) => +prev + +totalPrice);
  ...
  const seletedItemsData = useCallback((cart, btn, qty) => {
    if (selectedItemList.some((product) => product.productKey === cart.productKey)) {
      const price = cart?.price;
      if (btn === '+') {
        setSelectedItemList((prevList) =>
          prevList.map((product) =>
            product.productKey === cart.productKey ? { ...product, qty } : product
          )
        );
        setTotal((prev) => +prev + +price);
      } else if (btn === '-') {
