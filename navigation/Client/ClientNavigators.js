...
export const SellerNavigators = () => {
	...
	useEffect(() => {
		...
		const shopRef = child(dbRef, `shops/${authData.uid}`)
		const shopsRef = child(dbRef, `shops`)

		const refs = [shopRef, shopsRef]

		// wee need to get all the shops and products
		onValue(shopsRef, shopsSnapshot => {
			const rawShopsData = shopsSnapshot.val()

			const shopsDatas = Object.values(rawShopsData)

			const shops = []

			for (let i = 0; i < shopsDatas.length; i++) {
				const shopData = shopsDatas[i]

				// Iterate over each product using Object.entries()
				Object.entries(shopData).forEach(([shopKey, shop]) => {
					// Add the productKey to the product data
					shop['shopKey'] = shopKey
					if (shop.uid === authData.uid) {
						dispatch(setShopKey(shop.shopKey))
					}
					shops.push(shop)
				})

				// get product of a shop
				const productsRef = child(dbRef, `products/${shops[i]?.shopKey}`)
				refs.push(productsRef)
				const productRefAction = getProductOfaShop(productsRef)
				dispatch(productRefAction)

				// get orders id of a shop
				if (shops[i].shopKey === authData?.shopKey) {
					const shopordersRef = child(dbRef, `shopsOrders/${shops[i]?.shopKey}`)
					...
					onValue(shopordersRef, shopOrdersSnapShot => {
						...
						const orderIds = Object.values(ordersObjects)

						for (let index = 0; index < orderIds.length; index++) {
							const orderId = orderIds[index]

							const orderRef = child(dbRef, `orders/${orderId}`)
